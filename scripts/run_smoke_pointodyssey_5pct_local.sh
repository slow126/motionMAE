...
# Run 5% PointOdyssey smoke suite sequentially.
# Usage:
#   bash scripts/run_smoke_pointodyssey_5pct_local.sh
# Optional env:
#   PYTHON=python3
#   CUDA_DEVICES=0,1
#   LOG_DIR=./logs
...
CONFIGS=(
  "pointodyssey_smoke_random_5pct.yaml"
  "pointodyssey_smoke_heuristic_5pct.yaml"
  "pointodyssey_smoke_top_joint_5pct.yaml"
  "pointodyssey_smoke_top_mean_5pct.yaml"
...
for cfg in "${CONFIGS[@]}"; do
  ...
  if ! CUDA_VISIBLE_DEVICES="$CUDA_DEVICES" "$PYTHON" -u train_lightning.py --config "$cfg_path" "${resume_args[@]}" 2>&1 | tee "$log_path"; then
    ...
    if [[ "$CONTINUE_ON_ERROR" == "1" ]]; then
      echo "CONTINUE_ON_ERROR=1, continuing to next config..."
