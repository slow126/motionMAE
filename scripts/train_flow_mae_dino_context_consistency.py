...
from src.flow_mae.dino_context_consistency_lightning import (
    FlowMAEDINOContextConsistencyLightningModule,
)
...
def main() -> None:
    ...
    print(f"[train_flow_mae_dino_context_consistency] run_dir={run_dir}")
    ...
    if pointodyssey_probe_config and pointodyssey_probe_config.get("enabled", False):
        ...
        print(
            "[train_flow_mae_dino_context_consistency] "
    ...
    print(
        ...
        f"use_rgb_inputs={model_config.get('use_rgb_inputs', False)} "
        f"lambda_align={training_config.get('lambda_align', 0.05)} "
        f"dino_dim={dino_feature_dim} "
        f"dino_grid={dino_grid_hw}"
    )

    lightning_module = FlowMAEDINOContextConsistencyLightningModule(
        model_config=FlowMAEDINOContextConsistencyModelConfig(**model_config),
    ...
    if resume_ckpt:
        ...
        print(f"[train_flow_mae_dino_context_consistency] warm-starting model weights from {resume_ckpt}")
        ...
        missing, unexpected = lightning_module.model.load_state_dict(state, strict=False)
        if missing:
            print(f"  [warn] missing keys: {missing}")
        if unexpected:
            print(f"  [warn] unexpected keys: {unexpected}")
        print(f"  loaded {len(state)} tensors with strict=False — optimizer/LR schedule starts fresh")
