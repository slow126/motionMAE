...
class FlowMAEDINOContextModelConfig:
    ...
    observation_mask_mode: str = "scheduled"
    speckle_keep_ratio: float = 0.05
    speckle_dilation_kernel: int = 1
    scheduled_full_prob: float = 0.10
    scheduled_none_prob: float = 0.40
    loss: str = "smooth_l1"
...
class FlowMaskedAutoencoderDINOPrependedContextViT(nn.Module):
    def __init__(self, config: FlowMAEDINOContextModelConfig) -> None:
        super().__init__()
        self.config = config
    ...
    def flatten_context_tokens(self, context: torch.Tensor) -> torch.Tensor:
        ...
        if context.shape[2] != self.context_feature_dim:
            raise ValueError(
                f"Expected DINO feature dim {self.context_feature_dim}, got {context.shape[2]}."
            )
        return context
    ...
    def compute_loss(self, pred_flow: torch.Tensor, target_flow: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
