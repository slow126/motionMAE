...
class PFPascalDataset(CorrespondenceDataset):
    def __init__(
        ...
        self.src_bbox = []
        self.trg_bbox = []
        annotations = pickle.load(open(os.path.join(self.ann_path, 'annos.pkl'), 'rb'))
        for src_imname, trg_imname, cls in zip(self.src_imnames, self.trg_imnames, self.cls_ids):
            ...
            src_anns = annotations[self.cls[cls]][os.path.basename(src_imname).replace('jpg', 'mat')]
            trg_anns = annotations[self.cls[cls]][os.path.basename(trg_imname).replace('jpg', 'mat')]
            src_kp = torch.tensor(src_anns['kps']).float()
            trg_kp = torch.tensor(trg_anns['kps']).float()
            src_box = torch.tensor(src_anns['bbox'][0].astype(float))
            trg_box = torch.tensor(trg_anns['bbox'][0].astype(float))
        ...
        self.bidirectional_flows = bidirectional_flows
