"""
Driving Dataset (part of SceneFlow) for CATs++ training.

Uses torchvision.datasets.SceneFlowStereo with variant='Driving'.
"""
...
class DrivingSimpleDataset(Dataset, nn.Module):
    """
    Driving dataset from the SceneFlow dataset collection.
    ...
    def __init__(self, root: str, split: str, transforms: Optional[Callable] = None, reverse_flow: bool = False):
        ...
        Args:
            root: Path to SceneFlow dataset root (should contain 'Driving' subdirectory)
        ...
        self.dataset = datasets.SceneFlowStereo(root=root, variant='Driving', split=split, transforms=transforms)
