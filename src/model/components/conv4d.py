# Copied from https://github.com/ZhengyuLiang24/Conv4d-PyTorch/blob/main/Conv4d.py
...
class Conv4d(torch.nn.Module):
    def __init__(
        ...
        for i in range(self.kernel_size[0]):
            ...
            conv3d_layer.weight = torch.nn.Parameter(self.weight[:, :, i, :, :])
    ...
    def forward(self, input):
        ...
        out = torch.zeros(Batch, self.out_channels, l_o, d_o, h_o, w_o, device=input.device)
...
if __name__ == "__main__":
    ...
    net = Conv4d(1, 1, kernel_size=(3, 1, 1, 1), padding=0, stride=1, dilation=1, bias=True).cuda()
    out1 = net(input)